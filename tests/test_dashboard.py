...
def load_pytest_results(report_path="storage/reports/pytest_report.json"):
    path = Path(report_path)
    if not path.exists():
        return {}
    return json.loads(path.read_text(encoding="utf-8"))

def filter_functions(functions, only_missing=True):
    if only_missing:
        return [f for f in functions if f.get("docstring") is None]
    return functions

# from dashboard_ui.dashboard import load_pytest_results, filter_functions
# from core.reporter.dashboard import load_pytest_results, filter_functions
